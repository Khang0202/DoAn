...
public class WarningListAdapter extends RecyclerView.Adapter<WarningListAdapter.Viewholder> {
    ...
    public WarningListAdapter(List<Warning> items) {
        this.items = items;
    }

    @NonNull
    @Override
    public WarningListAdapter.Viewholder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View inflate = LayoutInflater.from(parent.getContext()).inflate(R.layout.viewholder_warning_list, parent, false);
        context = parent.getContext();
        return new WarningListAdapter.Viewholder(inflate);
    }

    @Override
    public void onBindViewHolder(@NonNull WarningListAdapter.Viewholder holder, int position) {
        ...
        holder.itemView.setOnClickListener(v -> {
            Intent intent = new Intent(holder.itemView.getContext(), ActiveWarningActivity.class);
            StaticClass.editWarning = items.get(position);
